...
const Rank = ["A", "K", "Q", "J", "T", "9", "8", "7", "6", "5", "4", "3", "2"] as const
...
const stringToHand = (str: string): Hand => {
  ...
  str.split('').forEach((card: Card) => hand[card]? hand[card]++:hand[card]=1)
...
const sortHand = (a: Hand, b: Hand): number => {
  ...
  if (a[1].length === 1 && b[1].length === 1) return sortCard(a[1][0][0], b[1][0][0])
...
const handToStr = ([_,a]: Hand): string => {
  return a.reduce((prev, [card, no]) => prev + card.repeat(no), "")
}
